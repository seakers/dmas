...
if __name__=="__main__":
    ...
    ground_track_dir = './utils/ground_tracks/existing'
    alt_ground_tracks = []
    tss_ground_tracks = []
                ...
                with open(f) as csv_file:
                    ...
                    for row in csv_reader:
                        ...
                        if(float(row[3]) > 180.0):
                            ...
                            lon = float(row[3])
                        tss_ground_tracks.append((float(row[2]),lon,float(row[0])*100))
            else:
                with open(f) as csv_file:
                    csv_reader = csv.reader(csv_file, delimiter=',')
                    i = 0
                    for row in csv_reader:
                        ...
                        if(float(row[3]) > 180):
                            ...
                            lon = float(row[3])
                        alt_ground_tracks.append((float(row[2]),lon,float(row[0])*100))
    coobs_floods = []
    ...
    for t in range(0,86400*16,1000):
        ...
        filename = f'./utils/images_existing/frame_{t}.png'
        ...
        if (t == 86400*16):
            plt.savefig("final_1b_existing.png",dpi=200)
        plt.close()
    print('Charts saved\n')
    gif_name = 'plot2D_1b_existing'
    # Build GIF
    print('Creating gif\n')
