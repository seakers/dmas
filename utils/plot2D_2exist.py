...
if __name__=="__main__":
    ...
    for filename in os.listdir(directory):
        ...
        if os.path.isfile(f):
            ...
            elif "all" in f and ("Sentinel-2" in f or "Landsat" in f):
                with open(f) as csv_file:
                    csv_reader = csv.reader(csv_file, delimiter=',')
                    i = 0
                    for row in csv_reader:
                ...
                with open(f) as csv_file:
                    ...
                    for row in csv_reader:
                        if(i == 0):
                            ...
                            continue
                        all_temps.append((row[0],row[1],row[2]))
            elif "all" in f and ("Jason" in f or "Cryo" in f or "SWOT" in f or "Sentinel-6" in f):
                with open(f) as csv_file:
                    csv_reader = csv.reader(csv_file, delimiter=',')
                    i = 0
                    for row in csv_reader:
    ...
    for filename in os.listdir(ground_track_dir):
        ...
        # checking if it is a file
        if os.path.isfile(f):
            if ("Sentinel-2" in f or "Landsat" in f):
                with open(f) as csv_file:
                    csv_reader = csv.reader(csv_file, delimiter=',')
                    i = 0
                    for row in csv_reader:
