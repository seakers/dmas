    ...
    for file in os.listdir(subdirpath):
        ...
        if os.path.isfile(fn) and ".nc" in fn and file[15:17]=="06":
            ...
            for pair in latlonpairs:
                ...
                row = (lats[pair[0]],lons[pair[1]],lake_temp,date)
                rows.append(row)
lakes = []
for point in points:
    current_lake = point
    ...
    for row in rows:
        if(np.sqrt((current_lake[0]-row[0])**2+(current_lake[1]-row[1])**2) < 1): # within 1 degree lat/lon distance
            lake_event = (current_lake[0], current_lake[1], row[2], row[3])
            lakes.append(lake_event)
            rows.remove(row)
with open('laketemps_june.csv','w') as out:
    csv_out=csv.writer(out)
    csv_out.writerow(['lat','lon','temp','date'])
    for lake in lakes:
